...
VRoid Studio v0.5.0での説明。VRoidやUnity用パッケージがころころ変わり、その結果もころころ変わるので
すぐに陳腐化するとは思いますが。

VRoidからVRoid.vrmでエクスポート
↓
...
VRoidから出力すると一般的なPMXモデルとサイズが異なる場合が多いので、PmxEditorで調整しよう。
これで、DxLibでも足は正常に表示できるPMXができた。

しかし、
・髪の毛などの色が違う
・髪の毛がだらーんと垂れる
・モデルによっては腕やスカートの透明テクスチャが適切に処理されない（Zバッファ問題）
があるのであれこれ対処しよう。（丸投げ）

ちなみにDxLib Model Viewerでの読み込みオプションは、
・法線再計算をON
・PMD/PMX読み込み時の物理演算のFPSは120
にしておこう。


剛体の xxxx_com をボーン追従
とか　お好みで調整　　ただし、DxLib Model Viewerは一回ごとに再起動した方がよい
...
DxLibはDirectShowを使用した動画再生に対応していますが、個人的にはDxLibがDirectShowではなくネイティブで対応しているOgg Theora (*.ogv) を使用しています。と言っても「DirectShowだとCodecの有無や種類によって再生できないとか、再生タイミングがずれる」等の問題によるものではなく、「DxLibでは、OGVでないとシークできない」という点によるものです。
