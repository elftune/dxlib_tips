/*
	2019/01/31	タイピングソフトサンプル by elftune
...
TCHAR checkInputChar(TCHAR c)
{
	...
	if (c != '\0' && c >= CTRL_CODE_CMP) {
		...
		*/
		...
		while (pData->InputChars[i] != NULL) // 1～数個あるので繰り返し
		{
			...
			if (TypingData.CompleteIndexNum > 0) {
				if (DxLib::strcmpDx(StringConvTable[TypingData.TableIndex[TypingData.CompleteIndexNum - 1]].BaseString, TEXT("ん")) == 0 && (nSubCounter == 0)) // 前の文字が ん の場合
				{
					...
					DxLib::strcatDx(acTemp, TypingData.CompletionString);
					DxLib::strcatDx(acTemp, TypingData.TypingString);

					/* ん は n, nn, xn, n' なので、2文字入力されていれば問題ない。つまり2文字前が n か x なら何も考えなくてよい*/
					bool b2 = true; // んとして nが1文字、しか入力されていないか
					if ((int)DxLib::strlenDx(acTemp) >= 2) // 調べるにしても2文字は必要だ
					{
						TCHAR c3 = acTemp[(int)DxLib::strlenDx(acTemp) - 2];
			...
			if (nSubCounter >= 0)
			{
				if (bN == false) c2 = *(pData->InputChars[i] + nSubCounter); // 本来入力すべき文字が c2
			}

			if ((c2 == c) || (c == '\'' && c2 == 'n' && nSubCounter != 0)) // 入力した文字が「どれかの途中段階」に一致か
			{
				bool b3 = true;
				if ((c2 == c) && (nSubCounter > 0))
				{
					// それまでに入力した文字同士が一致するか。たとえば うぉ は who や ulo があるが、wlo をOKにしてはいけない
					// 2文字目だけのチェックで一致していてもダメ。
					for (int u = 0; u < nSubCounter; u++)
					{
						if (*(pData->InputChars[i] + u) != TypingData.TypingString[u])
						{
							b3 = false;
							break;
						}
		...
		{
			TypingData.TypingString[(int)DxLib::strlenDx(TypingData.TypingString)] = c;
			DxLib::strcatDx(TypingData.CompletionString, TypingData.TypingString);
			::ZeroMemory(TypingData.TypingString, sizeof(TypingData.TypingString));
...
int WINAPI WinMain(_In_ HINSTANCE, _In_opt_ HINSTANCE, _In_ LPSTR, _In_ int)
{
	...
	TCHAR *pStrQTxt = TEXT("うぉそんなに結果重視ならじゃあ時間を測ろう"); // 全角文字のみにすること
	TCHAR *pStrKTxt = TEXT("うぉそんなにけっかじゅうしならじゃあじかんをはかろう");
...
LOOP_INIT:
	...
	while (DxLib::ProcessMessage() == 0)
	{
		...
		int x = 32;
		DxLib::DrawString(x, 96, TypingData.CompletionString, DxLib::GetColor(0, 255, 255)); // 入力済部分
		x += DxLib::GetDrawStringWidth(TypingData.CompletionString, (int)DxLib::strlenDx(TypingData.CompletionString));
		DxLib::DrawString(x, 96, TypingData.TypingString, DxLib::GetColor(0, 255, 255)); // 入力済部分
		x += DxLib::GetDrawStringWidth(TypingData.TypingString, (int)DxLib::strlenDx(TypingData.TypingString));

		if (cMiss != '\0') { // ミスった文字があるなら描く
			TCHAR acNG[2]; acNG[0] = cMiss; acNG[1] = '\0';
			DxLib::DrawString(x, 96, acNG, DxLib::GetColor(255, 0, 0));
		}
